...
const outfitList = async (req, res) => {
  try {
    const product = await Product.find({ catagory: 'BRIDE', softdelete: false });
...
const outfitGroom = async(req,res)=>{
  try {
    const product = await Product.find({ catagory: 'GROOM', softdelete: false });
    // const userData = await User.findOne({token:token})
    console.log(product)
    res.send({ product });
  } catch (error) {
